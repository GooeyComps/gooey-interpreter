...
class Make(List):
	...
	grammar = ["Make", "make"], blank, attr("type", MakeType), blank, attr("varname", VarName), optional(blank, "with", blank, attr("attributes",AttributeList))

class GooeySet(List):
	#grammar = "set", blank, attr("varname", VarName), attr("attributes",AttributeList), "."

 	grammar = ["Set", "set"], blank, attr("varname", VarName), attr("attributes",AttributeList)
