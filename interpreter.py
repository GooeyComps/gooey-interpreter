			...
			if(expr.__class__.__name__ == "Make"):
				if hasattr(expr, "type"):
					if (expr.type == "Window"):
						...
						self.checkVarname(expr,bindings)
						b = self.makeButton(self.window,expr,bindings)
						...
						self.checkVarname(expr,bindings)
						t = self.makeText(self.window, expr, bindings)
							...
							for item in expr.attributes:
								if(hasattr(item, 'options')):
									...
									if hasattr(item.title, 'var'):
										...
										ttl.configure(text=item.text.value)
...
-------------------- TEXT --------------------
	...
	def makeText(self,w,expr,bindings):
				...
				if hasattr(item, 'text'):
					if hasattr(item.text, 'var'):
						if (item.text.var in bindings):
							...
							self.error("No formatted text with that name.")
					else:
...
-------------------- BUTTONS --------------------
	...
	def makeButton(self,w,expr,bindings):
